...
<?php 
$image = $ProjectAlbums->getArticle($_GET['id']);
        ...
        <div id="content">
            ...
            <?php 
				...
				if($path != ''){
					...
					foreach($ProjectAlbums->getRules() as $idx => $rule){
            ...
            <div id="crop-source" data-width="<?=$width?>" data-height="<?=$height?>" data-type="<?=$type?>">
            	<img src="<?=$ProjectAlbums->upload_path.($_GET['source']?$_GET['source']:'original/').$path?>" />
            </div>
            <div id="crop-result">
            	<img src="<?=$ProjectAlbums->upload_path.$folder.$path?>" class="current" />
