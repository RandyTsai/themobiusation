...
<body>
	<div id="container">
        ...
        <div id="content">
            ...
            <div class="nested-sort-list title-list" data-sort-level="<?=$config['project']['category_level']?>">
				<?php
					$button = array();										
					if($config['project']['edit_category'])
						array_push($button, array('path'=>'category_edit.php','label'=>$Lang->_('Edit')));					
					if($config['project']['delete_category'])
						array_push($button, array('path'=>'process.php', 'param'=>'action=delete-category', 'label'=>$Lang->_('Delete'), 'class'=>'delete-button'));
				?>
				<?=$ProjectCategory->outputTree(0, array('show_desc' => true, 'button' => $button))?>
            </div>
        </div>
