...
function onend() {
	global $folder;
	global $AboutAlbums;
	@unlink($folder.'/'.$AboutAlbums->async_lock_file);
}
...
if($ref_url['host'] == $site_url['host'] && file_exists($AboutAlbums->async_lock_file)){
	while($AboutAlbums->asyncPendingNum() > 0){		
		$AboutAlbums->asyncMakeThumb();
	}
}
?>