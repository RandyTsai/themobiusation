...
function onend() {
	global $folder;
	global $GalleryAlbums;
	@unlink($folder.'/'.$GalleryAlbums->async_lock_file);
}
...
if($ref_url['host'] == $site_url['host'] && file_exists($GalleryAlbums->async_lock_file)){
	while($GalleryAlbums->asyncPendingNum() > 0){		
		$GalleryAlbums->asyncMakeThumb();
	}
}
?>